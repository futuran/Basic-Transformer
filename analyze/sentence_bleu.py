...
#         elif args.score == 999:
#             # ランダム
#             best_id = random.randint(0,len(sent_i_comet_score)-1)
#         elif args.score == 0:
#             # sentence-BLEU
...
def main():
    ...
    for sent_i in range(len(ref_txt)):
        ...
        if args.score == 1:
            ...
            # Score Q2 (train pplに基づく手法)
            score_Q = np.array([np.log2(args.pa)**topk + (args.ppl) * np.log2(len(lqmts)) + np.sum(lqmts) for topk, lqmts in enumerate(sent_i_lqmts)])
            score_Q = np.array([np.log2(args.ppl) * len(lqmts) + np.sum(lqmts) for topk, lqmts in enumerate(sent_i_lqmts)])
            best_id = score_Q.argmax(axis=0)
        elif args.score == 3:
            # Score Q3 (Length Penalthに基づく手法)
            score_Q = np.array([(np.log2(args.pa)**topk + np.sum(lqmts))/(len(lqmts) ** args.pr + 1e-6) for topk, lqmts in enumerate(sent_i_lqmts)])  # 類似度減衰あり
            ...
            # Score Q2とQ3の組み合わせ
            for topk, (lqmts, txt) in enumerate(zip(sent_i_lqmts, sent_i_txt)):
                # print(txt)
                # print(lqmts)
                print('{}\t{}'.format(np.log2(args.ppl) * len(txt.replace('@@ ', '').split()), np.sum(lqmts)))
            # exit()
            score_Q = np.array([(np.log2(args.ppl) * len(txt.replace('@@ ', '').split()) + np.sum(lqmts))/(len(txt.replace('@@ ', '').split()) ** args.pr + 1e-6) for topk, (lqmts, txt) in enumerate(zip(sent_i_lqmts, sent_i_txt))])
            best_id = score_Q.argmax(axis=0)
        elif args.score == 998:
            best_id = args.topk-1
        elif args.score == 0:
