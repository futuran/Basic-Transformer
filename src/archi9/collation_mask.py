...
class CollationAndMask:
    ...
    def collate_fn(self, batch):
        ...
        for x in batch:
            src_length = len(x['src'].split(' | ')[0].split())

            src_batch.append(
                self.vocab.text_transform['src'](x['src'].split()))
            tgt_batch.append(
                self.vocab.text_transform['tgt'](x['tgt'].split()))
            src_type_label.append(x['sim_rank'])
            src_length_mask_batch.append(torch.ones(src_length))
            match_batch.append(x['match'])
        ...
        src_length_mask_batch = pad_sequence(
            src_length_mask_batch, padding_value=0)[:, :-1]
