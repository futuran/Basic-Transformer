...
def greedy_decode(collation_mask, vocab, model, src, memory_list, max_len, start_symbol, device):
    # function to generate output sequence using greedy algorithm

    ys = torch.ones(1, 1).fill_(start_symbol).type(torch.long).to(device)
    # memory_list = memory_list.to(device)
    ...
    for i in range(max_len-1):
        ...
        out = model.decode_pred(ys, memory_list, tgt_mask)
...
def greedy_decode_moe(collation_mask, vocab, model, src, memory, max_len, start_symbol, device, num_src_sim):
    # function to generate output sequence using greedy algorithm
    ys = torch.ones(1, 1).fill_(start_symbol).type(torch.long).to(device)
    ...
    for i in range(max_len-1):
        ...
        prob = None
        for j in range(1, num_src_sim):
            current_memory = memory[:,j::num_src_sim, :]
