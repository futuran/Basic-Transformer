...
from src.archi7.collation_mask import *
from src.archi7.optimizer import *
from src.archi7.vocabs import *
from src.archi7.load_data import *
from src.archi7.transformer import *
from src.archi7.decode import *
...
def train_epoch(collation_mask: CollationAndMask, train_data, model, optimizer, loss_fn, loss_fn_for_sim, cfg: DictConfig, device):
    ...
    for src, tgt, src_type_label, src_length_mask in tqdm(train_dataloader):
...
def evaluate(collation_mask: CollationAndMask, dev_data, model, loss_fn, loss_fn_for_sim, cfg: DictConfig, device):
    model.eval()
    losses = 0
    ...
    if cfg.do_train:
        ...
        for epoch in range(1, cfg.ex.model.num_epochs + 1):
            ...
            train_loss = train_epoch(
                collation_mask, train_data, model, optimizer, loss_fn, loss_fn_for_sim, cfg, device)
            end_time = timer()
            val_loss = evaluate(collation_mask, dev_data, model, loss_fn, loss_fn_for_sim, cfg, device)
