...
def train_epoch(collation_mask: CollationAndMask, train_data, model, optimizer, loss_fn, cfg: DictConfig, device):
    ...
    for src, tgt, sent_idx in tqdm(train_dataloader):
        # src_text = " ".join(collation_mask.vocab.vocab_transform['src'].lookup_tokens(src.transpose(1,0)[0].numpy())).replace("<pad>", "")
        # tgt_text = " ".join(collation_mask.vocab.vocab_transform['tgt'].lookup_tokens(tgt.transpose(1,0)[0].numpy())).replace("<pad>", "")

        src = src.to(device)
        tgt = tgt.to(device)
        # sent_idx = sent_idx.to(device)

        current_batch_size = tgt.shape[1]
        ideal_batch_size = int(current_batch_size / collation_mask.num_of_sim)
        ...
        w = torch.repeat_interleave(torch.eye(
            ideal_batch_size, ideal_batch_size), collation_mask.num_of_sim, dim=0).to(device)

        logits = torch.matmul(logits.transpose(0, 2).transpose(
            1, 2), w).transpose(1, 2).transpose(0, 2)
        indices = torch.arange(0, current_batch_size,
                               collation_mask.num_of_sim).to(device)
        tgt_out = torch.index_select(tgt_out, 1, indices)
...
def evaluate(collation_mask: CollationAndMask, dev_data, model, loss_fn, cfg: DictConfig, device):
    ...
    for src, tgt, sent_idx in tqdm(dev_dataloader):
        src = src.to(device)
        tgt = tgt.to(device)
        # sent_idx = sent_idx.to(device)
...
def main(cfg: DictConfig):
    ...
    for ln in ['src', 'tgt']:
        vocab.text_transform[ln] = collation_mask.sequential_transforms(
            vocab.vocab_transform[ln],
            collation_mask.tensor_transform)  # Add BOS/EOS and create tensor
    ...
    if cfg.do_train:
        ...
        for epoch in range(1, cfg.ex.model.num_epochs + 1):
            ...
            end_time = timer()
            val_loss = evaluate(collation_mask, dev_data,
    ...
    if cfg.do_predict:
        ...
        if cfg.ex.load_checkpoint != '':
            model.load_state_dict(torch.load(
                cwd / Path(cfg.ex.load_checkpoint)))
        model.to(device)
        model.eval()
        ...
        for i, batch in enumerate(test_dataloader):
            ...
            tmp, q_mts = translate(collation_mask, vocab,
                                   model, batch['src'][0], device)
            out_list.append(tmp + '\n')
